...
public class CTFGameUI : MonoBehaviour {
    ...
    private void OnEnable()
    {
        ...
        EventManager.ShowFlagCapturedMessage += OnShowFlagCapturedMessage;
    ...
    private void OnShowFlagCapturedMessage(Team team) {
        Color textColor;
        string message;
        if (team == Team.RED) {
            textColor = Color.red;
            message = Localisation.GetString("RedTeamCaptured");
        }
        else {
            textColor = new Color(0.2f, 0.2f, 1f);
            message = Localisation.GetString("BlueTeamCaptured");
        }
        messagesQueue.Enqueue(new MessageInfo() { MessageText = message, TextColor = textColor });
    }
