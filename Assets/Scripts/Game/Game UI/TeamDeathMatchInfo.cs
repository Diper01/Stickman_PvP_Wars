...
public class TeamDeathMatchInfo : Photon.PunBehaviour {
    ...
    private void SetupTeamScore() {
        if (PhotonNetwork.room.CustomProperties.ContainsKey(RoomProperty.RedTeamScore))
            redTeamScore.text = Localisation.GetString("Score") + " " + (int)PhotonNetwork.room.CustomProperties[RoomProperty.RedTeamScore];
        else
            redTeamScore.text = Localisation.GetString("Score") + " 0"; 

        if (PhotonNetwork.room.CustomProperties.ContainsKey(RoomProperty.BlueTeamScore))
            blueTeamScore.text = Localisation.GetString("Score") + " " + (int)PhotonNetwork.room.CustomProperties[RoomProperty.BlueTeamScore];
        else
            blueTeamScore.text = Localisation.GetString("Score") + " 0";
    }
